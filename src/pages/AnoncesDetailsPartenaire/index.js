...
function AnnoncesDetailsPartenaire() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line
        axios.get(`http://localhost:8080/api/utilisateur/addedannonces/${user.id}`).then((secres1) => {
      // eslint-disable-next-line
  ...
  const handleInputChange = (event) => {
    ...
    if (value !== "" && value.length > 2) {
      axios.get(`http://localhost:8080/api/utilisateur/addedannonces/${user.id}`).then((bd) => {
        if (
